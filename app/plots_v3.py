...
def _load(csv_path: Path) -> pd.DataFrame:
    ...
    defaults = {
        ...
        # added 9/24
        "cost_per_query_usd": np.nan,
        "throughput_per_dollar_tokens_per_sec_per_usd_per_sec": np.nan,
        # end
    ...
    for c in (
        ...
        # NEW:
    "cost_per_query_usd","throughput_per_dollar_tokens_per_sec_per_usd_per_sec",
    ):
...
#added 9/24
def plot_cost_per_query(df: pd.DataFrame, figs_dir: Path):
    """Grouped bars: cost per query (USD) by precision, grouped by batch size."""
    fig, ax = plt.subplots(figsize=(8,5))
    precisions = [p for p in df["precision"].cat.categories if p in df["precision"].unique().tolist()]
    bszs = sorted(df["batch_size"].dropna().unique())
    x = np.arange(len(precisions))
    ...
    for i, b in enumerate(bszs):
        sub = df[df["batch_size"] == b]
        y = [sub[sub["precision"] == p]["cost_per_query_usd"].mean() for p in precisions]
        bars = ax.bar(x + (i - (len(bszs)-1)/2)*width, y, width=width,
                      label=f"bsz={int(b)}", **BAR_KW)
        _label_bars(ax, bars, fmt="%.2e")  # scientific notation is easier to read

    ax.set_xticks(x); ax.set_xticklabels(precisions)
    ax.set_ylabel("Cost per query (USD)")
    ax.set_title("Cost per Query by Precision")
    _legend_outside(ax)
    _savefig(fig, figs_dir, "cost_per_query_by_precision")


def plot_throughput_per_dollar(df: pd.DataFrame, figs_dir: Path):
    """
    Grouped bars: throughput per $ (tokens/sec ÷ $/sec) by precision, grouped by batch size.
    Column name is long: throughput_per_dollar_tokens_per_sec_per_usd_per_sec
    """
    fig, ax = plt.subplots(figsize=(8,5))
    precisions = [p for p in df["precision"].cat.categories if p in df["precision"].unique().tolist()]
    bszs = sorted(df["batch_size"].dropna().unique())
    x = np.arange(len(precisions))
    width = min(0.8 / max(1, len(bszs)), 0.25)
    col = "throughput_per_dollar_tokens_per_sec_per_usd_per_sec"

    for i, b in enumerate(bszs):
        sub = df[df["batch_size"] == b]
        y = [sub[sub["precision"] == p][col].mean() for p in precisions]
        bars = ax.bar(x + (i - (len(bszs)-1)/2)*width, y, width=width,
                      label=f"bsz={int(b)}", **BAR_KW)
        _label_bars(ax, bars, fmt="%.0f")  # whole-number style works well

    ax.set_xticks(x); ax.set_xticklabels(precisions)
    ax.set_ylabel("Throughput per $ (tokens/sec per $/sec)")
    ax.set_title("Throughput per Dollar by Precision")
    _legend_outside(ax)
    _savefig(fig, figs_dir, "throughput_per_dollar_by_precision")
...
def _have_cols(df, cols):
    ...
    if missing:
        print(f"[plot] Skipping: missing columns: {missing}")
        return False
    return True


def plot_cents_per_1k_tokens(df: pd.DataFrame, figs_dir: Path):
    """
    Grouped bars: ¢ per 1k tokens by precision, grouped by batch size.
    Lower is better.
    """
    if not _have_cols(df, ["cents_per_1k_tokens"]):
        return
    fig, ax = plt.subplots(figsize=(8,5))
    precisions = [p for p in df["precision"].cat.categories if p in df["precision"].unique().tolist()]
    bszs = sorted(df["batch_size"].dropna().unique())
    ...
    for i, b in enumerate(bszs):
        sub = df[df["batch_size"] == b]
        y = [sub[sub["precision"] == p]["cents_per_1k_tokens"].mean() for p in precisions]
        bars = ax.bar(x + (i - (len(bszs)-1)/2)*width, y, width=width,
                      label=f"bsz={int(b)}", **BAR_KW)
        _label_bars(ax, bars, fmt="%.3f")

    ax.set_xticks(x); ax.set_xticklabels(precisions)
    ax.set_ylabel("Cost (¢ per 1k tokens) — lower is better")
    ax.set_title("Cost per 1k Tokens by Precision")
    _legend_outside(ax)
    _savefig(fig, figs_dir, "cost_per_1k_tokens_by_precision")


def plot_tokens_per_usd_millions(df: pd.DataFrame, figs_dir: Path):
    """
    Grouped bars: tokens per $ (in millions) by precision, grouped by batch size.
    Higher is better.
    """
    if not _have_cols(df, ["tokens_per_usd_millions"]):
        return
    fig, ax = plt.subplots(figsize=(8,5))
    precisions = [p for p in df["precision"].cat.categories if p in df["precision"].unique().tolist()]
    bszs = sorted(df["batch_size"].dropna().unique())
    ...
    for i, b in enumerate(bszs):
        sub = df[df["batch_size"] == b]
        y = [sub[sub["precision"] == p]["tokens_per_usd_millions"].mean() for p in precisions]
        bars = ax.bar(x + (i - (len(bszs)-1)/2)*width, y, width=width,
                      label=f"bsz={int(b)}", **BAR_KW)
        _label_bars(ax, bars, fmt="%.2f")

    ax.set_xticks(x); ax.set_xticklabels(precisions)
    ax.set_ylabel("Tokens per $ (millions) — higher is better")
    ax.set_title("Tokens per Dollar by Precision")
    _legend_outside(ax)
    _savefig(fig, figs_dir, "tokens_per_usd_millions_by_precision")


def plot_queries_per_usd(df: pd.DataFrame, figs_dir: Path):
    """
    Grouped bars: queries per $ by precision, grouped by batch size.
    Higher is better. This reflects your *current workload shape*.
    """
    if not _have_cols(df, ["queries_per_usd"]):
        return
    fig, ax = plt.subplots(figsize=(8,5))
    precisions = [p for p in df["precision"].cat.categories if p in df["precision"].unique().tolist()]
    bszs = sorted(df["batch_size"].dropna().unique())
    ...
    for i, b in enumerate(bszs):
        sub = df[df["batch_size"] == b]
        y = [sub[sub["precision"] == p]["queries_per_usd"].mean() for p in precisions]
        bars = ax.bar(x + (i - (len(bszs)-1)/2)*width, y, width=width,
                      label=f"bsz={int(b)}", **BAR_KW)
        _label_bars(ax, bars, fmt="%.0f")

    ax.set_xticks(x); ax.set_xticklabels(precisions)
    ax.set_ylabel("Queries per $ — higher is better")
    ax.set_title("Queries per Dollar by Precision")
    _legend_outside(ax)
    _savefig(fig, figs_dir, "queries_per_usd_by_precision")


def plot_cost_vs_throughput_pareto(df: pd.DataFrame, figs_dir: Path):
    """
    Pareto view: cost vs throughput.
      x = ¢ per 1k tokens (lower is better)
      y = QPS (higher is better)
    Points are colored by precision and annotated with batch size.
    """
    if not _have_cols(df, ["cents_per_1k_tokens", "throughput_qps"]):
        return
    fig, ax = plt.subplots(figsize=(8,5))
    for p, sub in df.groupby("precision", observed=True):
        if pd.isna(p): 
            continue
        sub = sub.dropna(subset=["cents_per_1k_tokens", "throughput_qps"])
        ax.scatter(sub["cents_per_1k_tokens"], sub["throughput_qps"], label=str(p), alpha=0.8)
        for _, r in sub.iterrows():
            if pd.notna(r["cents_per_1k_tokens"]) and pd.notna(r["throughput_qps"]):
                ax.annotate(f"bsz={int(r['batch_size'])}",
                            (r["cents_per_1k_tokens"], r["throughput_qps"]),
                            fontsize=8, xytext=(4,4), textcoords="offset points")
    ax.set_xlabel("Cost (¢ per 1k tokens) — lower is better")
    ax.set_ylabel("Throughput (QPS) — higher is better")
    ax.set_title("Cost vs Throughput (Pareto: lower-left is best)")
    _legend_outside(ax)
    _savefig(fig, figs_dir, "cost_vs_throughput_pareto")


def plot_cost_savings_vs_fp32(df: pd.DataFrame, figs_dir: Path):
    """
    Savings vs FP32 (%), per precision × batch size.
    Uses dollars-per-1k-tokens to compute: (1 - cost / fp32_cost) * 100.
    Falls back to cents_per_1k_tokens if usd_per_1k_tokens is absent.
    """
    # Pick a cost column we have
    if "usd_per_1k_tokens" in df.columns:
        cost_col = "usd_per_1k_tokens"
    elif "cents_per_1k_tokens" in df.columns:
        # convert cents → dollars to keep math consistent
        df = df.copy()
        df["usd_per_1k_tokens"] = pd.to_numeric(df["cents_per_1k_tokens"], errors="coerce") / 100.0
        cost_col = "usd_per_1k_tokens"
    else:
        print("[plot] Skipping savings plot: need usd_per_1k_tokens or cents_per_1k_tokens")
        return

    # Baseline key: match FP32 at same batch_size (and same run_id if present)
    key_cols = ["batch_size"]
    if "run_id" in df.columns and df["run_id"].notna().any():
        key_cols = ["run_id", "batch_size"]

    # Build FP32 baselines
    fp32 = df[df["precision"].astype(str).str.lower() == "fp32"].groupby(key_cols, observed=True).agg(
        base_cost=(cost_col, "mean")
    )

    # Join baseline back
    merged = df.merge(fp32, how="left", left_on=key_cols, right_index=True)

    # Savings (%): positive means cheaper than FP32
    merged["cost_savings_pct"] = np.where(
        merged["base_cost"].notna() & (merged["base_cost"] > 0) & merged[cost_col].notna(),
        (1.0 - (merged[cost_col] / merged["base_cost"])) * 100.0,
        np.nan
    )

    # Plot grouped bars (per batch size)
    fig, ax = plt.subplots(figsize=(8,5))
    precisions = [p for p in df["precision"].cat.categories if p in df["precision"].unique().tolist()]
    bszs = sorted(df["batch_size"].dropna().unique())
    x = np.arange(len(precisions))
    ...
    for i, b in enumerate(bszs):
        sub = merged[merged["batch_size"] == b]
        # mean savings by precision at this batch size
        y = [sub[sub["precision"] == p]["cost_savings_pct"].mean() for p in precisions]
        # Ensure FP32 baseline shows as 0%
        y = [0.0 if str(p).lower()=="fp32" else (yy if pd.notna(yy) else 0.0) for p, yy in zip(precisions, y)]
        bars = ax.bar(x + (i - (len(bszs)-1)/2)*width, y, width=width,
                      label=f"bsz={int(b)}", **BAR_KW)
        _label_bars(ax, bars, fmt="%.1f")

    ax.axhline(0.0, linewidth=1, linestyle="--", color="#888888")
    ax.set_xticks(x); ax.set_xticklabels(precisions)
    ax.set_ylabel("Savings vs FP32 (% per 1k tokens)  — higher is better")
    ax.set_title("Cost Savings vs FP32 Baseline")
    _legend_outside(ax)
    _savefig(fig, figs_dir, "cost_savings_vs_fp32")
...
def main():
    ...
    #added
    plot_cost_per_query(df, figs_dir)
    plot_throughput_per_dollar(df, figs_dir)
    #end
    ...
    # Cost/$ insight plots
    plot_cents_per_1k_tokens(df, figs_dir)
    plot_tokens_per_usd_millions(df, figs_dir)
    plot_queries_per_usd(df, figs_dir)
    plot_cost_vs_throughput_pareto(df, figs_dir)
    # NEW: show savings vs FP32
    plot_cost_savings_vs_fp32(df, figs_dir)
