...
def _add_overall_geo_means(df_stats: pd.DataFrame) -> pd.DataFrame:
    """
    Adds per-precision geometric mean speedup across ALL batches for:
      - tokens_per_sec
      - throughput_qps
    Expects columns: ['precision','metric','ratio_precision_over_baseline'].
    """
    def _gm(vals):
        a = np.asarray([v for v in vals if np.isfinite(v) and v > 0.0], dtype=float)
        return float(np.exp(np.mean(np.log(a)))) if a.size else np.nan

    col = "ratio_precision_over_baseline"

    gm_tok = (
        df_stats[df_stats["metric"] == "tokens_per_sec"]
        .groupby("precision", observed=True)[col]
        .apply(_gm)
    )
    gm_qps = (
        df_stats[df_stats["metric"] == "throughput_qps"]
        .groupby("precision", observed=True)[col]
        .apply(_gm)
    )

    df_stats = df_stats.copy()
    df_stats["gm_tokens_per_sec_all_batches"] = df_stats["precision"].map(gm_tok)
    df_stats["gm_throughput_qps_all_batches"] = df_stats["precision"].map(gm_qps)
    return df_stats
...
def main():
    ...
    out = _add_overall_geo_means(out) 
